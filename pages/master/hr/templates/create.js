...
export default function AddTemplateDialog({ fetchItems }) {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      setSubmitting(true);
      const res = await fetch(`${BASE_URL}/hr/templates`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      if (data.statusCode === 200) {
        const dto = data.result ?? data.Result;
        const newId = dto?.id ?? dto?.Id;
        if (pendingSignatureFile && newId) {
          const fd = new FormData();
          fd.append("signature", pendingSignatureFile);
          const upRes = await fetch(
            `${BASE_URL}/hr/templates/${newId}/signature?orgId=${orgId}`,
            {
              method: "POST",
          ...
          if (upData.statusCode !== 200) {
            toast.warning(
              upData.message ||
                "Template saved but signature upload failed. Edit the template to add a signature."
            );
          }
        }
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} sm={6}>
                    <FormControl fullWidth size="small" required>
                      <InputLabel id="template-create-type-label">Type</InputLabel>
                      <Select
                        labelId="template-create-type-label"
                        id="template-create-type-select"
                        label="Type"
                        value={values.Type}
                        onChange={(e) => {
                  ...
                  <Grid item xs={12} sm={6}>
                    <FormControl fullWidth size="small">
                      <InputLabel id="template-create-company-label">Company</InputLabel>
                      <Select
                        labelId="template-create-company-label"
                        id="template-create-company-select"
                        label="Company"
                        value={
                          values.CompanyId === "" ||
                  ...
                  {isDocumentTemplateType(values.Type) && (
                    ...
                    <Grid item xs={12}>
                      <TemplateSignatureSection
                        ...
                        onPendingSignatureChange={setPendingSignatureFile}
                      />
                    </Grid>
                    </>
                  )}
