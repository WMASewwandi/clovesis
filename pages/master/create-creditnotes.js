...
const CNNCreate = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (parseFloat(creditAmount) >= (selectedInvoice?.totalInvoiceAmount || 0) || parseFloat(creditAmount) < 0) {
      toast.error("Credit Amount must be less than the Invoice Outstanding Amount.");
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (jsonResponse.result.result !== "") {
          ...
          setTimeout(() => {
            window.location.href = "/master/CCN";
          }, 3000);
        } else {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (jsonResponse.result && jsonResponse.result.length > 0) {
          setInvoicesForCustomer(jsonResponse.result.map(item => ({
  ...
  const handleCreditAmountChange = (e) => {
    ...
    if (newCreditAmount >= (selectedInvoice?.totalInvoiceAmount || 0) || newCreditAmount < 0) {
      setCreditAmountError("Credit Amount must be less than the Invoice Outstanding Amount.");
    } else {
  ...
  useEffect(() => {
    ...
    // if (invoiceList) {
    //   setInvoice(invoiceList[0]?.documentNo || "");  
    // }
  ...
  return (
    <>
      ...
      <div className={styles.pageTitle}>
        ...
        <ul>
          <li>
            <Link href="/master/Customer Credit Notes">Customer Credit Notes</Link>
          </li>
          <li>Customer Credit Notes Create</li>
        </ul>
      </div>
        ...
        <Grid item xs={12} sx={{ background: "#fff" }}>
          <Grid container p={1}>
            ...
            <Grid item xs={12} lg={6} display="flex" flexDirection="column">
              ...
              <Grid item xs={12} display="flex" flexDirection="column" mt={1} mb={1}>
              ...
              <Grid item xs={12} display="flex" justifyContent="space-between" mt={0} mb={10}>
                  <Typography
                    component="label"
                    sx={{
                      fontWeight: "500",
              ...
              <Grid container>
                ...
                <Grid item xs={12} display="flex" alignItems="center" justifyContent="space-between" mt={1}>
                  ...
                  <Autocomplete
                    ...
                    value={selectedInvoice ? { invoiceNumber: selectedInvoice.invoiceNumber } : null}
                    onChange={(event, newValue) => {
                    ...
                    renderOption={(props, option) => (
                      <li {...props}>
                        <Typography variant="body2">
                          {`${option.invoiceNumber} / ${option.totalInvoiceAmount}`}
                        </Typography>
                      </li>
                    )}
                  />
                  ...
                  <Grid item xs={12} display="flex" alignItems="center" justifyContent="space-between" mt={1}>
                    ...
                    <TextField
                      ...
                      size="small"
                      fullWidth
                      value={selectedInvoice ? `${selectedInvoice.totalInvoiceAmount}` : ""}
            ...
            <Grid item xs={12} my={3} sx={{ display: "flex", justifyContent: "flex-end" }}>
              <Button
                variant="contained"
                color="primary"
                sx={{
