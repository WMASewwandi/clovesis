...
import { Checkbox, FormControlLabel, Grid, IconButton, Tooltip, Typography, MenuItem, Select, FormControl, InputLabel, FormHelperText } from "@mui/material";
...
export default function EditVehicle({ item, fetchItems }) {
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    ...
    formData.append("ImmageOrPdfUpload", item.immageOrPdfUpload || "");
    ...
    try {
      const response = await fetch(`${BASE_URL}/Vehicle/UpdateVehicle`, {
        method: "PUT",
        body: formData,
        headers: {
        ...
        <Box sx={style}>
          <Formik
            initialValues={{
              ...
              VehicleType: item.vehicleType !== undefined ? item.vehicleType : "",
              VehicleNumber: item.vehicleNumber || "",
              Description: item.description || "",
              IsActive: item.isActive !== undefined ? item.isActive : true,
              IsSold: item.isSold !== undefined ? item.isSold : false,
            }}
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} md={6}>
                    <Button
                      ...
                      sx={{ height: '56px' }}
                    >
                      {file ? file.name : (item.immageOrPdfUpload ? "Change Image/PDF" : "Upload Image/PDF")}
                      <input
                        type="file"
                        hidden
