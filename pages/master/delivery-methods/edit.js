...
const mapItemToInitialValues = (item) => ({
  Id: item?.id ?? null,
  Name: item?.name ?? "",
  ...
  WeightBasedCharge: item?.weightBasedCharge ?? "",
  Description: item?.description ?? "",
  IsActive: Boolean(item?.isActive),
  SupportsCOD: Boolean(item?.supportsCOD),
  ImageURL: null,
  ExistingImageUrl: item?.imageURL ?? "",
});

export default function EditDeliveryMethodModal({ item, fetchItems, deliveryTypeOptions: externalOptions }) {
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      const response = await fetch(`${BASE_URL}/DeliveryMethods/UpdateDeliveryMethod`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      if (!response.ok || data.statusCode !== 200) {
        throw new Error(data.message || "Failed to update delivery method");
      }

      toast.success(data.message || "Delivery method updated successfully.");
      handleClose();
      if (typeof fetchItems === "function") {
        fetchItems();
      }
    } catch (error) {
      toast.error(error.message || "Unable to update delivery method");
    } finally {
      setSubmitting(false);
    }
  };

  const initialValues = useMemo(() => mapItemToInitialValues(item), [item]);
                ...
                <Box sx={{ flexGrow: 1, overflowY: "auto", pr: 1, minHeight: 0 }}>
                  ...
                  <Grid item xs={12} md={6}>
                    ...
                    <FormControl fullWidth size="small" error={touched.DeliveryType && Boolean(errors.DeliveryType)}>
                      ...
                      <Field as={Select} label="Delivery Type" name="DeliveryType" value={values.DeliveryType}>
