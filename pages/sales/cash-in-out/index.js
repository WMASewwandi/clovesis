...
export default function CashInOut() {
  ...
  const handleConfirmApprove = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${BASE_URL}/Shift/ApproveCashInOutStatusAsync?id=${selectedRecord.id}`,
        {
          method: "POST",
          headers: {
  ...
  const handleConfirmReject = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `${BASE_URL}/Shift/RejectCashInOutStatusAsync?id=${selectedRecord.id}&reason=${encodeURIComponent(rejectReason.trim())}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <>
      ...
      <div className={styles.pageTitle}>
        ...
        <ul>
          <li>
            <Link href="/sales/cash-in-out/">Cash In/Out</Link>
          </li>
        </ul>
      </div>
      <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 1, md: 1, lg: 1, xl: 2 }}>
        <Grid item xs={12} lg={4} order={{ xs: 2, lg: 1 }}>
          <Search className="search-form">
            <StyledInputBase
              placeholder="Search by Shift, Warehouse or Cash Flow Type.."
      ...
      <Dialog open={approveModalOpen} onClose={handleCloseApprove} maxWidth="xs" fullWidth>
        <DialogTitle>Approve Cash In/Out</DialogTitle>
      ...
      <Dialog open={rejectModalOpen} onClose={handleCloseReject} maxWidth="xs" fullWidth>
        <DialogTitle>Reject Cash In/Out</DialogTitle>
        <DialogContent dividers>
