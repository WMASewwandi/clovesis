...
export default function SalesOrderPrintPage() {
  ...
  useEffect(() => {
    if (!router.isReady || !salesOrderId) {
      return;
    }
    ...
    const fetchSalesOrder = async () => {
      try {
        ...
        const response = await fetch(
          `${BASE_URL}/SalesOrder/GetSalesOrderById?id=${salesOrderId}`,
          {
            method: "GET",
            headers: {
  ...
  useEffect(() => {
    ...
    const warehouseId =
      localStorage.getItem("warehouse") || salesOrderData?.warehouseId;
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      const pageElements = contentRef.current.querySelectorAll(
        '[data-sales-order-pdf-page="true"]'
      );
  ...
  const renderTable = (items, isLastPage) => (
    <Box sx={{ mb: { xs: 2, sm: 3 }, borderTop: "2px solid #333", borderBottom: "2px solid #333" }}>
      ...
      >
        ...
        <Box sx={{ flex: 1.2, textAlign: "right", fontSize: { xs: "0.52rem", sm: "0.78rem" } }}>Unit Price</Box>
    ...
    <Box>
      ...
      <Box sx={{ borderTop: "2px solid #333", borderBottom: "2px solid #333", py: 1, mb: 2 }}>
        <Typography sx={{ fontWeight: "bold", textAlign: "center", fontSize: { xs: "1rem", sm: "1.5rem" }, lineHeight: 1.2 }}>
          {pageIndex === 0 ? "SALES ORDER" : "SALES ORDER (CONT.)"}
        </Typography>
      </Box>
        ...
        >
          <Box sx={{ flex: 1 }}>
            {[
              ...
              ["Document No", salesOrderData?.documentNo || documentNumber || "-"],
              ["Order Date", formatDisplayDate(salesOrderData?.documentDate)],
            ].map(([label, value]) => (
          ...
          <Box sx={{ flex: 1 }}>
            {[
              ["Warehouse", salesOrderData?.warehouseName || warehouseData?.name || "-"],
              ["Sales Person", salesOrderData?.salesPersonName || "-"],
        ...
        <Box mb={5} ref={contentRef}>
          {loadingSalesOrder ? (
            ...
            salesOrderPages.map((items, pageIndex) => {
              ...
              return (
                <Box
                  key={`sales-order-page-${pageIndex}`}
                  data-sales-order-pdf-page="true"
                  sx={{
                    width: { xs: "100%", sm: "210mm" },
                    minHeight: { xs: "auto", sm: "297mm" },
