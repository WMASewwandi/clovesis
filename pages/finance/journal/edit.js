...
export default function EditJournalEntry({ fetchItems, item }) {
  ...
  const handleSubmit = (values) => {
    ...
    fetch(`${BASE_URL}/JournalEntry/UpdateJournalEntry`, {
      method: "POST",
  ...
  const initialValues = {
    JournalDate: formatDate(item?.journalDate) || new Date().toISOString().split("T")[0],
    ReferenceNo: item?.referenceNo || "",
    Narration: item?.narration || "",
    ApprovedBy: item?.approvedBy ? String(item.approvedBy) : "",
    Status: item?.status !== undefined && item?.status !== null ? String(item.status) : statuses[0]?.value || "1",
    JournalEntryLines:
      item?.journalEntryLines && item.journalEntryLines.length > 0
        ? item.journalEntryLines.map((line) => ({
