...
export default function EditVendorInvoice() {
  ...
  const fetchVendorInvoice = async () => {
    try {
      ...
      const response = await fetch(`${BASE_URL}/VendorInvoice/ReadVendorInvoice?id=${id}`, {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        throw new Error("Failed to fetch vendor invoice");
      }
      ...
      if (invoice) {
        const lines = invoice.vendorInvoiceLines || [];
        setFormValues({
  ...
  // Update supplier when suppliers are loaded and form has supplierId
  useEffect(() => {
    if (suppliers.length > 0 && formValues.supplierId && !selectedSupplier) {
      const supplier = suppliers.find((s) => s.id === parseInt(formValues.supplierId));
      if (supplier) {
        setSelectedSupplier(supplier);
      }
    }
  }, [suppliers, formValues.supplierId, selectedSupplier]);
  ...
  const handleSubmit = async (event) => {
    ...
    if (!id) {
      toast.error("Missing vendor invoice identifier.");
      return;
    }
    if (formValues.vendorInvoiceLines.length === 0) {
      toast.error("At least one invoice line is required.");
      return;
    }

    // Validate all lines have account
    const invalidLines = formValues.vendorInvoiceLines.filter((line) => !line.accountId);
    ...
    const payload = {
      ...
      VendorInvoiceLines: formValues.vendorInvoiceLines.map((line) => ({
    ...
    try {
      setSubmitting(true);
      const response = await fetch(`${BASE_URL}/VendorInvoice/UpdateVendorInvoice`, {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData?.message || "Failed to update vendor invoice");
      }

      const data = await response.json();
      toast.success(data?.message || "Vendor invoice updated successfully.");
      navigateToBack();
    } catch (error) {
      toast.error(error.message || "Unable to update vendor invoice");
    } finally {
      setSubmitting(false);
    }
  };
