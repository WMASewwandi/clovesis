...
export default function MaterialReturnNotes() {
  const router = useRouter();
  const cId = sessionStorage.getItem("category");
  const { navigate, create, update, remove, print } = IsPermissionEnabled(cId);
  const name = localStorage.getItem("name");
  const { data: ReportName } = GetReportSettingValueByName("MaterialReturnNote");
  ...
  const [returnNotesList, setReturnNotesList] = React.useState([]);

  React.useEffect(() => {
    if (returnNotes && returnNotes.length > 0) {
      setReturnNotesList(returnNotes);
    }
  }, [returnNotes]);
        ...
        <Grid item xs={12} order={{ xs: 3, lg: 3 }}>
          <TableContainer component={Paper}>
            <Table aria-label="simple table" className="dark-table">
              ...
              <TableBody>
                {returnNotesList.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={10} align="center">
                      <Typography color="error">
                        No Material Return Note Available
                      </Typography>
                    </TableCell>
                  </TableRow>
                ) : (
                  returnNotesList.map((item, index) => {
