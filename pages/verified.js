...
export default function EmailVerified() {
  ...
  useEffect(() => {
    const verifyEmail = async () => {
      // Wait for router to be ready and get accountId from query
      if (!router.isReady) {
        return;
      }
      ...
      if (!accountId) {
        // Keep loading if accountId is missing
        return;
      }
      ...
      try {
        ...
        const response = await fetch(
          `${BASE_URL}/CRMAccounts/VerifyEmail?accountId=${accountId}`,
          {
            method: "POST",
            headers: headers,
          }
    ...
    verifyEmail();
  }, [router.isReady, router.query.accountId]);
