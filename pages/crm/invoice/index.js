...
export default function InvoiceList() {
  ...
  const handleConfirmDelete = async () => {
    if (!selectedInvoice?.id) {
      toast.error("Unable to determine invoice to delete.");
      return;
    }
  ...
  const handleConfirmSend = async () => {
    if (!selectedInvoice?.id) {
      toast.error("Unable to determine invoice to send.");
      return;
    }
    ...
    <>
      ...
      <Grid container rowSpacing={2} columnSpacing={{ xs: 1, sm: 1, md: 1, lg: 1, xl: 2 }}>
        ...
        <Grid item xs={12} lg={6} mb={1} display="flex" justifyContent="flex-end" order={{ xs: 1, lg: 2 }}>
          {create && (
            <Link href="/crm/invoice/create" passHref legacyBehavior>
                  ...
                  invoices.map((invoice) => {
                    ...
                    return (
                      <TableRow key={invoice.id}>
                        <TableCell>{invoice.invoiceNo || `Invoice #${invoice.id}`}</TableCell>
                        <TableCell>
                          {invoice.quoteNumber || "-"}
                        </TableCell>
                        <TableCell>
                          ...
                          {update && (
                            <Tooltip title="Edit">
                              <IconButton
                                size="small"
                                component={Link}
                                href={`/crm/invoice/edit?id=${invoice.id}`}
      ...
      <Dialog open={isDeleteDialogOpen} onClose={handleCloseDialog} maxWidth="xs" fullWidth>
        ...
        <DialogContent dividers>
          <DialogContentText>
            ...
            <strong>{selectedInvoice ? selectedInvoice.invoiceNo || `invoice #${selectedInvoice.id}` : "this invoice"}</strong>? This action cannot be undone.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={isSendDialogOpen} onClose={handleCloseSendDialog} maxWidth="xs" fullWidth>
        ...
        <DialogContent dividers>
          <DialogContentText>
            ...
            <strong>{selectedInvoice ? selectedInvoice.invoiceNo || `invoice #${selectedInvoice.id}` : "this invoice"}</strong> to the customer via email?
          </DialogContentText>
        </DialogContent>
        <DialogActions>
