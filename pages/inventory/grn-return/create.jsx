            ...
            <Grid item gap={2} xs={12} display="flex" justifyContent="end">
              {returnType === "grn" && (
                <Button
                  ...
                  onClick={handleReset}
                >
                  <Typography sx={{ fontWeight: "bold" }}>Reset</Typography>
                </Button>
              )}
                 ...
                 onChange={(event, newValue) => {
                    if (newValue) {
                      const updatedList = newValue.goodReceivedNoteLineDetails.map(item => {
                        // For each item from the GRN, find its current stock level
                        ...
                        // Create a new row object using the CURRENT stock qty
                        return {
                          ...item,
                          qty: currentStockQty, // CRITICAL: Use live stock qty, not old GRN qty
                          isSelected: false,
                          returnedQty: 0,
                          returnAmount: 0,
                          remark: '',
                      ...
                      // Handle case where GRN is cleared
                      setSelectedGrnProduct(null);
                      setSelectedRows([]);
                    }
                  }}
            ...
            {returnType === 'non-grn' && (
              <Grid item xs={12} mt={2} mb={1}>
                ...
                <Autocomplete
                  ...
                  getOptionLabel={(option) => option.name || ""}
                  loading={isSearching}
                  disabled={!supplier || !currentUserWarehouse}
                  filterOptions={(x) => x}
                  ...
                  renderOption={(props, option) => {
                    const stockInfo = stockBalances.find(b => b.productId === option.id);
                    const stockQty = stockInfo ? stockInfo.bookBalanceQuantity : 0;
                        ...
                        <Grid container alignItems="center" sx={{ width: '100%' }}>
                          <Grid item xs={8}>
                            ...
                            <Typography variant="body2" color="text.secondary">
                              {option.categoryName} {option.subcategoryName && `> ${option.subcategoryName}`} | UOM: {option.uomName}
                            </Typography>
                          </Grid>
                  ...
                  renderInput={(params) => (
                    <TextField
                      {...params}
                      placeholder={
                        !supplier ? "Select a supplier first" : !currentUserWarehouse ? "User warehouse not found" : "Click to see all items or type to search..."
                      }
                      InputProps={{
                        ...params.InputProps,
                        endAdornment: (
                ...
                >
                  <TableHead>
                    <TableRow sx={{ background: "#757fef" }}>
                      ...
                      <TableCell sx={{ color: "#fff" }}>Stock Quantity</TableCell>
