...
export default function Summary() {
  ...
  const navToPrev = () => {
    if (from) {
      ...
      router.push({
        pathname: `/inquiry/edit-inquiry/tshirt/document-panel/`,
        query: { id: inquiry ? inquiry.inquiryId : "",option: inquiry ? inquiry.optionId : "" },
      });
    }
  };
  ...
  const handleSaveValues = () => {
    ...
    const bodyData = {
      ...
      UnitProfit: parseFloat(formData.profit),
      TotalProfit: parseFloat(formData.totalProfit),
      SellingPrice: parseFloat(formData.sellingPrice),
      Revanue: parseFloat(formData.revenue),
      ApprovedStatus: 0,
