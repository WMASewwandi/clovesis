...
export default function EditWorkTrackModal({ open, onClose, workTrack, fetchItems }) {
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      const response = await fetch(`${BASE_URL}/WorkTrack/UpdateWorkTrack`, {
        method: "POST",
        headers: {
      ...
      if (response.ok && (data.status === "SUCCESS" || data.statusCode === 200)) {
        toast.success("Work Track updated successfully!");
        onClose();
        fetchItems();
      } else {
        const errorMessage = data.message || data.error || data.errorMessage || "Failed to update work track";
        toast.error(errorMessage);
      }
    } catch (error) {
      toast.error(error.message || "An error occurred while updating the work track");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <>
      <Modal open={open} onClose={onClose} aria-labelledby="edit-work-track-modal">
        <Box sx={style}>
          <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
            <Typography id="edit-work-track-modal" variant="h6" component="h2">
              Edit Work Track
            </Typography>
          ...
          <Formik
            initialValues={{
              customerId: workTrack?.customerId || null,
              projectId: workTrack?.projectId || null,
              remarks: workTrack?.remarks || "",
            }}
                  ...
                  <Grid item xs={12}>
                    <Box sx={{ display: "flex", gap: 2, justifyContent: "flex-end", mt: 2 }}>
                      ...
                      <Button type="submit" variant="contained" disabled={isSubmitting}>
                        {isSubmitting ? "Updating..." : "Update Work Track"}
                      </Button>
                    </Box>
                  </Grid>
                </Grid>
