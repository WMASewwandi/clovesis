...
export default function PurchaseOrderNotesSummaryReport({ docName, reportName }) {
  const warehouseId = localStorage.getItem("warehouse");
  const [open, setOpen] = useState(false);
  const [fromDate, setFromDate] = useState("");
  const [toDate, setToDate] = useState("");
  const { data: purchaseOrderNotesSummaryReport } = GetReportSettingValueByName(reportName);
