...
export default function EditOngoingSize({ fetchItems, inquiry, sizeItem }) {
  ...
  const inqId = inquiry ? inquiry.ongoingInquiryId : null;
  const inqCode = inquiry ? inquiry.inquiryCode : "";
  const optionName = inquiry ? inquiry.optionName : "";
  const OptId = inquiry ? inquiry.optionId : null;
  const WinType = inquiry ? inquiry.windowType : null;
  const inqType = inquiry ? inquiry.windowType : null;
  ...
  const handleOpen = () => {
    if (sizeItem) {
      // Pre-populate form with existing size data
      setTwoXSValue(sizeItem.twoXS || 0);
      setXsValue(sizeItem.xs || 0);
      setSValue(sizeItem.s || 0);
      setMValue(sizeItem.m || 0);
      setLValue(sizeItem.l || 0);
      setXLValue(sizeItem.xl || 0);
      setTwoXLValue(sizeItem.twoXL || 0);
      setThreeXLValue(sizeItem.threeXL || 0);
      setFourXLValue(sizeItem.fourXL || 0);
      setFiveXLValue(sizeItem.fiveXL || 0);
      setLength(sizeItem.length || 0);
      setWidth(sizeItem.width || 0);
      setQuantity(sizeItem.totalQty || 0);
      setTotal(sizeItem.totalQty || 0);
      setFinalTotal(sizeItem.totalQty || 0);
      setValue(sizeItem.isSpecialSize ? 1 : 0);
    }
    setOpen(true);
  };
  ...
  const handleSubmitNormal = (values) => {
    const intValues = {
      Id: sizeItem ? sizeItem.id : 0, // Use existing ID for update, 0 for new
      InquiryID: inqId,
      InqCode: inqCode,
      WindowType: WinType,
      OptionId: OptId,
      InqOptionName: optionName,
      TwoXS: parseFloat(twoXSValue) || 0,
      XS: parseFloat(xSValue) || 0,
      S: parseFloat(sValue) || 0,
      M: parseFloat(mValue) || 0,
      L: parseFloat(lValue) || 0,
      XL: parseFloat(xLValue) || 0,
      TwoXL: parseFloat(twoXLValue) || 0,
      ThreeXL: parseFloat(threeXLValue) || 0,
      FourXL: parseFloat(fourXLValue) || 0,
      FiveXL: parseFloat(fiveXLValue) || 0,
      TotalQty: parseFloat(total) || 0,
      Length: parseFloat(length) || 0,
      Width: parseFloat(width) || 0,
      IsSpecialSize: false,
      SizeID: values.SizeID ? parseInt(values.SizeID) : (sizeItem ? sizeItem.sizeID : null),
      SizeName: values.SizeName || (sizeItem ? sizeItem.sizeName : ""),
      Sleavetype: values.Sleavetype || (sizeItem ? sizeItem.sleavetype : ""),
    };
    const token = localStorage.getItem("token");
    fetch(`${BASE_URL}/Ongoing/CreateOrUpdateOngoingSize`, {
      method: "POST",
      ...
      .then((data) => {
        if (data.statusCode == 200) {
          ...
          handleClose();
          if (inquiry) {
            fetchItems(inquiry.ongoingInquiryId, inquiry.optionId, inquiry.windowType);
          } else {
            fetchItems();
          }
        } else {
      ...
      .catch((error) => {
        toast.error(error.message || "Size update failed. Please try again.");
      });
  };
  ...
  const handleCalcTotal = (value, size) => {
    ...
    const sum =
      parseFloat(newTwoXSValue || 0) +
      parseFloat(newXSValue || 0) +
      parseFloat(newSValue || 0) +
      parseFloat(newMValue || 0) +
      parseFloat(newLValue || 0) +
      parseFloat(newXLValue || 0) +
      parseFloat(newTwoXLValue || 0) +
      parseFloat(newThreeXLValue || 0) +
      parseFloat(newFourXLValue || 0) +
      parseFloat(newFiveXLValue || 0) +
  ...
  const handleCalcQuantity = (value, size) => {
    ...
    const sum = parseFloat(newLength || 0) + parseFloat(newWidth || 0);
        ...
        <Box sx={style} className="bg-black">
          <Box sx={{ width: "100%" }}>
            ...
            <TabPanel value={value} index={0}>
              <Formik
                initialValues={{
                  SizeName: sizeItem ? sizeItem.sizeName : "",
                  SizeID: sizeItem ? sizeItem.sizeID : "",
                  Sleavetype: sizeItem ? sizeItem.sleavetype : "",
                  TwoXS: sizeItem ? sizeItem.twoXS : "",
                  XS: sizeItem ? sizeItem.xs : "",
                  S: sizeItem ? sizeItem.s : "",
                  M: sizeItem ? sizeItem.m : "",
                  L: sizeItem ? sizeItem.l : "",
                  TwoXL: sizeItem ? sizeItem.twoXL : "",
                  ThreeXL: sizeItem ? sizeItem.threeXL : "",
                  FourXL: sizeItem ? sizeItem.fourXL : "",
                  FiveXL: sizeItem ? sizeItem.fiveXL : "",
                  TotalQty: sizeItem ? sizeItem.totalQty : "",
                  InquiryID: inqId,
                  InqCode: inqCode,
                  WindowType: WinType,
                  OptionId: OptId,
                  InqOptionName: optionName,
                  IsSpecialSize: false,
                  Width: sizeItem ? sizeItem.width : "",
                  Length: sizeItem ? sizeItem.length : "",
                }}
