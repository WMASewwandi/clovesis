...
const SearchCustomer = forwardRef(({
  ...
  const getCustomerLabel = (customer) => {
    ...
    return customer.displayName || customer.firstName || "-";
  };
  ...
  const handleSearch = async (value) => {
    try {
      ...
      const url = buildQuery(`${BASE_URL}/Customer/GetAllManufactureCustomers`, { keyword: keyword });
        ...
        >
          <List>
            {results.map((customer, index) => (
              ...
              >
                <ListItemText
                  primary={getCustomerLabel(customer)}
                />
              </ListItem>
            ))}
          </List>
